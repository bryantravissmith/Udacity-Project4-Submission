...
for i in range(len(features)):
    for j in range(i+1,len(features)):
        ...
        #       features_list = [features[i],features[j],features[k],features[m]]
                ...
                total_labels = []
                kf = KFold(len(data2), n_folds=10)
                #train_labels, test_labels, train_features, test_features = train_test_split(labels,temp.values,test_size=0.2,random_state=42)
                ...
                #if nb_r_score > 0.5 and nb_p_score > 0.5:
                if nb_r_score > 0.35 and nb_p_score > 0.35:
                    print "NB",features[i],features[j], nb_r_score, nb_p_score
                    #print "NB", features[i],features[j],features[k],features[m], nb_r_score, nb_p_score
...
for i in range(len(features2)):
    for j in range(i+1,len(features2)):
        ...
        features_list = [features2[i],features2[j],'from_this_person_to_poi_ratio','from_poi_to_this_person_ratio']
        #features_list = [features2[i],features2[j],'from_this_person_to_poi_ratio']
        #features_list = [features2[i],features2[j],'from_poi_to_this_person_ratio']
        ...
        if r_score > 0.4 and p_score > 0.4:
            print "TREE",features[i],features[j], r_score, p_score
            #print features[i],features[j],features[k], r_score, p_score
...
dump_classifier_and_data(clf, data, features_list)
